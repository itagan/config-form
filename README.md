...
- `pnpm test:schema`：校验远程 Schema 白名单解析与本地组件/Slot 绑定边界。
- `pnpm test:e2e`：在 Chromium 中验证校验聚焦、键盘导航、左右 Slot 和动态字段交互。
...
- `pnpm release:check`：执行发布元数据、Lint、覆盖率、Schema、类型、浏览器交互、构建、文档、同站校验和 npm 打包预检；不会发布。
