...
import type { ConfigFormHintTooltipRef } from '../utils/elementTooltipAdapter'
...
export type { ConfigFormHintTooltipRef } from '../utils/elementTooltipAdapter'

interface UseConfigFormHintTooltipOptions {
  containerRef: Readonly<Ref<HTMLElement | null>>
  tooltipRef: Ref<ConfigFormHintTooltipRef | null>
  content: Ref<string>
}

/** 在当前 ConfigForm 根节点委托提示事件，并协调目标、展示与生命周期模块。 */
export function useConfigFormHintTooltip(options: UseConfigFormHintTooltipOptions) {
  ...
  const syncActiveTarget = (forcePositionUpdate = false) => {
    ...
    const content = target?.getAttribute(CONFIG_FORM_HINT_ATTRIBUTE) || ''
    if (!target || !content) {
      presenter.hide()
      return
    }
