import type { ConfigFormNavigationOptions } from '../types'
import { CONFIG_FORM_FIELD_PROP_ATTRIBUTE } from './useConfigFormFieldLocator'

interface ConfigFormKeyboardNavigationOptions {
  getOptions: () => ConfigFormNavigationOptions | undefined
...
export function useConfigFormKeyboardNavigation(options: ConfigFormKeyboardNavigationOptions) {
  const handleNavigationKeydown = (event: KeyboardEvent) => {
    ...
    const currentField = event.target.closest<HTMLElement>(`[${CONFIG_FORM_FIELD_PROP_ATTRIBUTE}]`)
